      ...
      <div className="w-full max-w-md z-10 my-8">
        ...
        <div className="flex flex-col items-center mb-8 gap-3">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 font-medium">
            Start collaborating with MedChat today
          </p>
        </div>
        ...
        <div className="glass-panel rounded-3xl shadow-xl p-8 border border-slate-200/50 dark:border-white/5">
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            {/* Error */}
            {error && (
              <div className="bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 text-xs px-4 py-3 rounded-xl font-medium animate-fade-in">
                {error}
              </div>
            )}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                placeholder="doctor@medicalhub.com"
                required
                className="w-full px-4 py-3 rounded-xl border border-slate-200/60 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 text-slate-800 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-blue-500 dark:focus:border-blue-500/40 text-sm transition-all"
              />
            </div>
              ...
              <div className="flex gap-1.5 mt-1.5 px-0.5">
                {[1, 2, 3].map((i) => (
                  <div key={i} className={`h-1.5 flex-1 rounded-full transition-colors duration-300 ${
                    password.length === 0 ? "bg-slate-200 dark:bg-white/10"
                    : password.length < 6 && i === 1 ? "bg-red-500/80"
                    : password.length >= 6 && i <= 2 ? "bg-amber-500/80"
                    : password.length >= 10 ? "bg-emerald-500/85"
                    : "bg-slate-200 dark:bg-white/10"
                  }`} />
                ))}
              </div>
              <p className="text-[10px] pl-1 font-semibold text-slate-400 dark:text-slate-500">
                {password.length === 0 ? "Enter a password" 
                  : password.length < 6 ? "Too short"
                  : password.length < 10 ? "Medium security"
            ...
            <p className="text-[10px] text-center text-slate-400 dark:text-slate-500 leading-normal border-t border-slate-100 dark:border-white/5 pt-3">
              By creating an account, you acknowledge that MedChat is for <strong>research and informational purposes only</strong> and not a substitute for diagnostic medical consultation.
            </p>
          </form>
